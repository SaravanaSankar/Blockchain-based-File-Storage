...
import requests
from flask import render_template, redirect, request, send_file, url_for, render_template_string, make_response
import bcrypt
...
def all_users_name():
    ...
    return [user[0] for user in mycursor.fetchall()]
...
def index():
    get_tx_req()
    all_users = all_users_name()
    return render_template("index.html", all_users=all_users, title="FileStorage", subtitle="A Decentralized Network for File Storage/Sharing", node_address=ADDR, request_tx=request_tx)
...
def submit():
    if 'name' in session:
        ...
        # get the list of selected users from the form
        permission = request.form.getlist('users[]')
        # uploader name in sesson onbject
        ...
        post_object = {
            ...
            "permission": permission,    #list of users with access to the file
            "uploader": uploader
        }
