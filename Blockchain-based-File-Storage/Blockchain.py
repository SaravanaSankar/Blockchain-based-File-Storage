    ...
    def mine(self):
        if (len(self.pending) > 0):  # if there is atleast one pending transaction
            ...
            for transaction in self.pending:
                for access in transaction["permission"]:
                    transactions.append(access)
            for uploader in self.pending:
                giver = uploader["uploader"]
                print(giver)

            new_block = Block(last_block.index + 1, self.pending, last_block.hash, transactions, giver)
            ...
            self.pending = []
            # Returns the index of the block that was added to the chain
